-- Create centralized ticker universe table
...
-- Policies: public read, restrict writes to authenticated (edge functions with service role bypass RLS)
create policy if not exists "Ticker universe is publicly readable"
  on public.ticker_universe for select
  using (true);

create policy if not exists "Authenticated users can insert tickers"
  on public.ticker_universe for insert to authenticated
  with check (true);

create policy if not exists "Authenticated users can update tickers"
...
-- Seed data from current STOCK_UNIVERSE (100 symbols)
