-- Drop and recreate v_home_kpis view to fix data types and update logic
DROP VIEW IF EXISTS v_home_kpis CASCADE;

CREATE VIEW v_home_kpis AS
WITH dates AS (
...
trades_summary AS (
  SELECT 
    t.mode,
    COUNT(CASE WHEN t.status = 'OPEN' THEN 1 END)::integer as open_positions,
    ...
    COUNT(CASE WHEN t.status = 'CLOSED' AND t.exit_ts >= CURRENT_DATE - INTERVAL '30 days' THEN 1 END)::integer as closed_30d,
